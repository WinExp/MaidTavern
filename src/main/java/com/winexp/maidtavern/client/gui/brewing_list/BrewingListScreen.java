...
import com.winexp.maidtavern.network.serverbound.SetBrewingListC2SMessage;
...
public class BrewingListScreen extends AbstractContainerScreen<BrewingListScreen.BrewingListMenu> {
    ...
    public BrewingListScreen(LocalPlayer player, InteractionHand hand, BrewingList brewingList) {
        super(new BrewingListMenu(hand, player.level().registryAccess(), brewingList, Utils.getAllRecipesFor(player.level().getRecipeManager(), ModRecipes.BARREL_RECIPE)),
    ...
    private void onEntryButtonClicked(BrewingListEntry entry) {
        ...
        if (hasShiftDown()) {
            ...
            if (config.barrelPos().isEmpty()) {
                minecraft.pushGuiLayer(new InventorySelectionScreen(minecraft.player, stack -> stack.is(MaidTavernItems.BARREL_SELECTION_TOOL.get()), stack -> {
                    Set<BlockPos> targetPositions = MaidTavernItems.BARREL_POSITIONS_DATA.get(stack);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scroll) {
        ...
        if (leftArea.contains((int) mouseX, (int) mouseY) && menu.canSelectedScroll()) {
            ...
            menu.selectedScrollTo((int) Mth.clamp(menu.selectedScrollRow - scroll, 0, extraRows));
            return true;
        } else if (rightArea.contains((int) mouseX, (int) mouseY) && menu.canRecipeScroll()) {
            int extraRows = menu.getRecipeRows() - PICKER_ROWS;
            menu.recipeScrollTo((int) Mth.clamp(menu.recipeScrollRow - scroll, 0, extraRows));
            return true;
        }
    ...
    public static class BrewingListMenu extends AbstractContainerMenu {
        ...
        public BrewingListMenu(InteractionHand hand, RegistryAccess registries, BrewingList brewingList, List<RecipeHolder<BarrelRecipe>> allRecipes) {
            super(null, 0);
            this.hand = hand;
        ...
        public int getScrolledSelectedIdx(int slot) {
            ...
            selectedScrollRow = Mth.clamp(selectedScrollRow, 0, extraRows);
        ...
        public int getScrolledRecipeIdx(int slot) {
            ...
            recipeScrollRow = Mth.clamp(recipeScrollRow, 0, extraRows);
        ...
        public void removed(Player player) {
            ...
            SetBrewingListC2SMessage message = new SetBrewingListC2SMessage(hand, newList);
            MaidTavernNetworking.sendToServer(message);
        }
